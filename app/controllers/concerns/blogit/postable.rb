module Blogit::Postable
  ...
  def index
    @posts = Blogit::Post.order('created_at DESC').page(params[:page]).per(Blogit.configuration.posts_per_page)
  end
  ...
  protected
  def valid_params
    params.require(:post).permit(:title, :excerpt, :body, :blogger_type, :blogger_id, :tag_list)
  end
